...
- **Removed a HA Core deprecation warning** — the device cleanup that runs when you disable Schedule Calendar was calling a Home Assistant device-registry lookup that's deprecated and stops working in HA 2027.8. Switched to the current replacement. No behavioural change.

### Bug fixes

#### HomeKit

- **Turning a zone back on and setting a new temperature right after could still send it back to Off a few seconds later** ([#335](https://github.com/hiall-fyi/tado_ce/issues/335) - @davidjirovec) — v4.4.0 already fixed the plainer version of this: writing a temperature to an off zone through HomeKit only stores the number and doesn't turn the thermostat on, so that write goes through the cloud instead, which can. A second command sent right after the first, the way some automations restore heat once a window closes, could still slip through on HomeKit: it checked the zone's own not-yet-confirmed mode rather than what Tado's cloud had actually confirmed, so it took the HomeKit path anyway and reverted once nothing backed it up. Both commands now wait for that confirmation.
- **HomeKit setup no longer walks a bridgeless home to a PIN entry screen** — turning on HomeKit with no v3 or v3+ Internet Bridge among your zones (a home with only Smart AC Control units, or a v2 GW-prefix bridge) used to lead straight to a PIN prompt for a device the pairing scan could never find, ending in "please try again" either way. It now says why up front and lets you back out, instead of a dead end dressed up as a login screen.
- **A HomeKit bridge that reconnects on its own now clears its own repair notice** — after a bridge lost its pairing, Tado CE raises a notice suggesting a factory reset and a re-pair. An ordinary automatic reconnect that fixed the pairing by itself (an HA restart, a retry) used to leave that notice sitting there regardless, still asking you to redo a step you no longer needed. A successful reconnect now clears it, the same as re-pairing manually already did.

#### Zone lifecycle and state

- **Removing a zone no longer wipes every zone's Smart Comfort history** — v4.4.0 fixed this same bug for per-zone settings (Smart Valve Control mode, external sensors, temperature limits): take a radiator off your Tado account or delete a zone in the Tado app, and the integration tidied its saved data too hard, wiping everything rather than trimming the one zone that left. The same bug was still there for Smart Comfort's saved temperature history, used for heating-rate estimates and preheat timing. Nothing looked wrong at the time, because the running integration carried on using what it already had in memory; the history only disappeared at the next restart. History belonging to a zone that no longer exists is now left alone; nothing reads it, so it costs you nothing.
- **A failed temperature, mode, fan, swing, timer, or operation-mode change no longer leaves a snapshot behind for Restore Previous State to wrongly apply later** — every one of those actions saves the zone's state first, so you can put it back with the Restore Previous State service. Several of them saved that snapshot before knowing whether the change actually went through, so a write Tado rejected still left a snapshot sitting there, ready to be restored onto a zone that never actually changed in the first place. Every one of those actions now only saves the snapshot once the change is confirmed.
- **A zone with no type reported by Tado no longer creates a full heating entity** — if a zone came back from Tado's API with no type at all, the integration treated it as a heating zone anyway: a live thermostat entity, plus every heating sensor. It's skipped now instead, the same way every other place in the integration already handles a zone whose type is missing.
- **A zone_id Tado reuses for a different room no longer risks restoring the previous room's saved state onto it** — a service like Restore Previous State snapshots a zone's state so it can be put back later. If Tado reassigns that zone_id to a different physical room (a rare event, seen after some re-pairs), the snapshot stayed attached to the old room's data. It's now cleared the moment Tado CE detects the reassignment, the same way the zone's other settings already were.

#### Smart Valve Control

- **Valve Target could send a leftover correction to a zone that had already gone back to its schedule** — a cloud write held back by the per-zone rate limit or a Tado quota pause used to queue itself and go out on the next check regardless of what had happened in between. If the zone reached target and handed control back to Tado's schedule in that window, the held-back write still landed anyway, briefly pulling the zone off schedule again. It's not held and replayed any more: if the correction's still wanted, the very next check works it out fresh; if the zone's already back on schedule, nothing gets sent.
- **Offset Sync no longer lurches while a zone is off** ([#343](https://github.com/hiall-fyi/tado_ce/issues/343) - @Si-Hill, [#345](https://github.com/hiall-fyi/tado_ce/issues/345) - @jokinzengotita) — with an external temperature sensor configured, Offset Sync could occasionally send a large, several-degree correction to the TRV every poll while a zone was off, audible as the valve motor moving, sometimes at night. It now moves the offset by at most a degree (or your own configured minimum-change threshold, if you've set that higher) per correction, so a shaky reading produces a small nudge instead of a sudden jump. Doesn't affect heating: the correction only keeps the app's displayed temperature accurate while a zone is off, same as always.

#### Quota and notices

- **The quota-reserve pause no longer promises manual control it can't deliver** — the log line for a paused sync said temperature and mode changes would still work through the pause. True once HomeKit local control is already serving cached data, but false the one time it mattered most: a reload or restart landing while quota is already at the reserve floor, before any entity exists yet to receive that change. It now says plainly that entities are unavailable until quota resets or the first data arrives, on that specific path.
- **The rate-limited notice now clears itself on every home, not just some** — after Tado rate-limits a request, Tado CE raises a notice and clears it once syncing succeeds again. On any home running without HomeKit configured, it never actually cleared on its own: the clearing check was tied to a flag a different startup path sets, one that home's syncs never take. It now clears the same way regardless of how HomeKit is set up.

#### Smart Comfort

- **Comfort Level and Smart Comfort Target could jump by up to 2°C right at the turn of a season, with no outdoor temperature sensor bound** — both compare your room to a target that's meant to track the weather gradually. Bind an outdoor sensor and that's exactly what happens; without one, the fallback target was keyed to the calendar month, so the same room temperature could read as noticeably more or less comfortable purely because midnight ticked over into a new season, four times a year. The fallback now drifts smoothly with the day of the year instead of jumping on fixed dates.
...
#### Zone settings that wouldn't stick
